 ...
 * Set DB_TYPE=postgres and DATABASE_URL in your environment for prod.
 */
...
function makeSqliteAdapter() {
  ...
  // Wrap in a Postgres-compatible async interface
  return {
    async query(sql, params = []) {
      ...
      if (isSelect) {
        ...
        // Return last inserted id as rows[0].id for INSERT … RETURNING
        return { rows: [{ id: info.lastInsertRowid, ...Object.fromEntries(params.map((_,j)=>['p'+j,params[j]])) }] };
      }
    },
    ...
    // Expose raw sqlite db for schema setup
    _raw: db,
    type: 'sqlite'
  };
}
...
async function initSchema(db) {
  ...
  if (db.type === 'sqlite') {
    // Execute all statements at once via exec
    db._raw.exec(schema);
  } else {
  ...
  for (let i = 0; i < ids.length; i++) {
    if (db.type === 'postgres') {
      await db.query(
        ...
        [ids[i], captions[i], locations[i], gradients[i]]
      );
    } else {
      db._raw.prepare(
        `INSERT INTO posts (user_id,caption,location,image_gradient) VALUES (?,?,?,?)`
      ).run(ids[i], captions[i], locations[i], gradients[i]);
    }
  }
